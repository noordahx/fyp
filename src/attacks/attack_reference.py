...
class ReferenceAttack:
    ...
    def train_reference_models(self, full_dataset, n_models=5):
        """
        Train or load reference models.
        """
        model_save_dir = os.path.join(self.config.output.save_dir, "reference_models")
        os.makedirs(model_save_dir, exist_ok=True)

        ref_models = []
        total_indices = np.arange(len(full_dataset))
        train_size = self.config.attack.population.reference_size
        ...
        for i in range(n_models):
            ...
            if os.path.exists(save_path):
                ...
                trainer(ref_model, ref_train_loader, ref_train_loader, self.config, save_path)
